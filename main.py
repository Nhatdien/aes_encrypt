...
def encrypt(state, key):
    if len(key) not in [16, 24, 32]:
        messagebox.showerror("Error", "Cipher key must be 16, 24, or 32 bytes.")
        return "", 0
    ...
    cypherkey = np.transpose(key_matrix).tolist() if len(key) != 16 else key_matrix

    start_time = time.time()
    encrypted_text = "".join(matToString(func[len(key)](stringToMat(i), cypherkey)) for i in res)
...
def decrypt(state, key):
    ...
    start_time = time.time()
    decrypted_text = "".join(matToString(func[len(key)](stringToMat(i), cypherkey)) for i in res)
...
def on_decrypt():
    decrypted_text, decrypt_time = decrypt(entry_encrypted.get(), entry_key.get())
...
style = {"bg": "#34495e", "fg": "white", "font": ("Arial", 18), "padx": 10, "pady": 10}

# UI Elements
tk.Label(form_frame, text="Plain Text:", **style).grid(row=0, column=0, padx=20, pady=10)
...
tk.Label(form_frame, text="Cipher Key:", **style).grid(row=1, column=0, padx=20, pady=10)
...
tk.Button(form_frame, text="Encrypt", command=on_encrypt, bg="#1abc9c", fg="white", font=("Arial", 18)).grid(row=2, column=0, columnspan=2, pady=20)

tk.Label(form_frame, text="Encrypted Text:", **style).grid(row=3, column=0, padx=20, pady=10)
entry_encrypted = tk.Entry(form_frame, width=60, font=("Arial", 18))
entry_encrypted.grid(row=3, column=1, padx=20, pady=10)

tk.Button(form_frame, text="Decrypt", command=on_decrypt, bg="#e74c3c", fg="white", font=("Arial", 18)).grid(row=4, column=0, columnspan=2, pady=20)

tk.Label(form_frame, text="Decrypted Text:", **style).grid(row=5, column=0, padx=20, pady=10)
